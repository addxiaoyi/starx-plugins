...
| 命令 | 说明 | 使用场景 |
|------|------|----------|
| `/register <密码> [邮箱]` | 注册账号 | 首次进入服务器时使用 |
| `/login <密码>` | 登录账号 | 每次进入服务器时使用 |
| `/2fa <验证码>` | 二步验证 | 开启 TOTP 后登录时使用 |
...
**工作流程**：玩家首次进入时用 `/register <密码>` 注册，之后用 `/login <密码>` 登录。如果开启 TOTP，登录后还需输入 `/2fa <验证码>`。
