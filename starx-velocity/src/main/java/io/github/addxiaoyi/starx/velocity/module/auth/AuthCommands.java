...
public final class AuthCommands implements VelocityModule {
  ...
  private final class TwoFactorCommand implements SimpleCommand {
    ...
    public void execute(Invocation invocation) {
      if (!(invocation.source() instanceof Player player)) {
        invocation.source().sendMessage(
            Component.text("此命令仅限玩家使用", NamedTextColor.RED));
        return;
      }
      ...
      switch (args[0].toLowerCase()) {
        case "enable" -> {
          if (args.length < 2) {
            invocation.source().sendMessage(
                Component.text("用法: /2fa enable <密码>", NamedTextColor.RED));
            return;
          }
          AuthResult result = authService.enableTotp(
              player.getUniqueId(), args[1]);
          if (result.success()) {
            invocation.source().sendMessage(
                Component.text("§a二步验证已开启！", NamedTextColor.GREEN));
            invocation.source().sendMessage(
                Component.text("请使用 Google Authenticator 或同类 App 扫描下方密钥：",
                    NamedTextColor.YELLOW));
            invocation.source().sendMessage(
                Component.text(result.message().substring(result.message().indexOf("密钥:")),
                    NamedTextColor.AQUA)
                    .clickEvent(ClickEvent.copyToClipboard(
                        extractSecret(result.message()))));
            invocation.source().sendMessage(
                Component.text("§c请务必保存此密钥，丢失后无法找回！", NamedTextColor.RED));
          } else {
            invocation.source().sendMessage(
                Component.text("§c" + result.message(), NamedTextColor.RED));
          }
        }
        case "disable" -> {
          if (args.length < 2) {
            invocation.source().sendMessage(
                Component.text("用法: /2fa disable <密码>", NamedTextColor.RED));
            return;
          }
          AuthResult result = authService.disableTotp(
              player.getUniqueId(), args[1]);
          if (result.success()) {
            invocation.source().sendMessage(
                Component.text("§a二步验证已关闭", NamedTextColor.GREEN));
          } else {
            invocation.source().sendMessage(
                Component.text("§c" + result.message(), NamedTextColor.RED));
          }
        ...
        case "status" -> {
          boolean enabled = authService.isTotpEnabled(
              player.getUniqueId());
    ...
    private void showUsage(Invocation invocation) {
      invocation.source().sendMessage(
          Component.text("===== 二步验证 (2FA) =====", NamedTextColor.GOLD));
      invocation.source().sendMessage(
      ...
      invocation.source().sendMessage(
          Component.text("§c开启后每次登录需要输入验证码，请妥善保存密钥", NamedTextColor.RED));
    }
